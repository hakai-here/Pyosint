...
def parse_args():
    ...
    # the main send_req need to be redef parse_args():
    # parse the arguments
    parser = argparse.ArgumentParser(epilog='\tExample: \r\npython ' + sys.argv[0] + " -d google.com")
    parser.error = parser_error
    parser._optionals.title = "OPTIONS"
    ...
    parser.add_argument('-o', '--output', help='Save the results to text file')
    parser.add_argument('-n', '--no-color', help='Output without color', default=False, action='store_true')
    return parser.parse_args()written
    def send_req(self, url):
        try:
            self.headers.update({'X-ApiKey':self.apikey})
            resp = self.session.get(url, headers=self.headers, timeout=self.timeout)
        except Exception as e:def parse_args():
    # parse the arguments
    parser = argparse.ArgumentParser(epilog='\tExample: \r\npython ' + sys.argv[0] + " -d google.com")
    parser.error = parser_error
    parser._optionals.title = "OPTIONS"
...
def callin():
    domain = "example.com"
    threads = 7000
    ports = None #port set value 
    verbose = None #to set foo set verbose false
    enable_bruteforce=False # to activate set enable bruteforce is None
    bur = vermanager(enable_bruteforce)
    savefile = None #name of the file 
    engines=None
    eng = "all"
    ...
    while True:
        ...
        if(inlist[0] == "help"):
            ...
            if(inlist[1] == "domain"):
                domain = inlist[2]
            elif(inlist[1] == "thread"):
                threads = int(inlist[2])
            ...
            elif(inlist[1] == "engine"):
                    if(inlist[2] == "all" ):
                        engines = None
                        eng = engmanager(engine)
                    else:
                        engines = inlist[2]
